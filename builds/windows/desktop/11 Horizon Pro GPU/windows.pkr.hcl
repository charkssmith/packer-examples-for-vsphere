...
locals {
  ...
  vm_name            = "${var.vm_guest_os_family}-${var.vm_guest_os_name}-${var.vm_guest_os_version}-${var.vm_guest_os_edition}-v${local.build_version}-${local.extra_info}"
...
build {
  ...
  provisioner "powershell" {
    ...
    elevated_user     = var.build_username
    elevated_password = var.build_password
    scripts           = formatlist("${path.cwd}/%s", ["scripts/windows/osotgpu.ps1"])
  }
