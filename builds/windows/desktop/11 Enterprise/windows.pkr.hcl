...
locals {
  ...
  vm_name_sds       = "${var.vm_guest_os_family}-${var.vm_guest_os_name}-${var.vm_guest_os_version}-${var.vm_guest_os_edition}-${local.build_date}-SDS"
...
source "vsphere-iso" "windows-desktop-11-sds" {
...
provisioner "powershell" {
    ...
    elevated_user     = var.build_username
    elevated_password = var.build_password
    scripts           = formatlist("${path.cwd}/%s", ["scripts/windows/teams.ps1"])
  }
